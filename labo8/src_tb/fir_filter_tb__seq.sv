...
// Description : FIR Filter Testbench - Pipeline version
//==============================================================================
...
module fir_filter_tb;
  ...
  //--------------------------------------------------------------------------
  // Task : envoi d'une donnée
  //--------------------------------------------------------------------------
  task send_sample(input signed [DATASIZE-1:0] sample);
  begin
    @(negedge clk_i);
    din_valid_i = 1'b1;
    din_i       = sample;
  ...
  //--------------------------------------------------------------------------
  // Task : attente + vérification résultat
  //--------------------------------------------------------------------------
  task wait_and_check(input signed [DATASIZE-1:0] expected);
  begin
    do begin
      @(posedge clk_i);
    end while (dout_valid_o !== 1'b1);
  ...
  initial begin
    ...
    // Fin
    //----------------------------------------------------------------------
    $display("[PASSED] FIR sequentiel test successful.");
    $finish;
  end

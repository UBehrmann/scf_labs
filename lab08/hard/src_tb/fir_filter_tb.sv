...
module fir_filter_tb;
  ...
  initial begin
    ...
    end else if (ARCH == ARCH_SEQUENTIAL) begin
    // ----------------------------------------------------------
    // -- TEST SEQUENTIEL 
    // ----------------------------------------------------------
      int n;
      ...
      while ((n < 20) || (expected_q.size() > 0)) begin
        ...
        if (dout_valid_o && dout_ready_i) begin
          if (expected_q.size() == 0) begin
            $error("[FAILED] Unexpected sequential output at time %0t", $time);
            $fatal;
          end
          ...
          if ($signed(dout_o) !== expected) begin
            $error("[FAILED] Sequential mismatch at time %0t: expected %0d, got %0d",
